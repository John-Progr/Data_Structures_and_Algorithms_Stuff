
/*
 * 
PriorityQueue.java
 */
import java.util.Comparator;

public class PQ<T> {
    /**
     * Array based heap representation
     */
    private T[] heap;
    /**
     * The number of objects in the heap
     */
    private int size;
    /**
     * Comparator.
     */
    protected Comparator<T> cmp;
	
	public int[] Songposition;//the array we use for song id in remove method
	
	
    
    
    /**
     * Creates heap with a given capacity and comparator.
     * param capacity The capacity of the heap being created.
     * param cmp The comparator that will be used.
     */
    public PQ(int capacity, Comparator<T> cmp) {
        if (capacity < 1) throw new IllegalArgumentException();
        
        this.heap = (T []) new Object[capacity+1];
        this.size = 0;
        this.cmp = cmp;
		this.Songposition=new int[9999];
    }
	
	public int size(){
		return size;
	}
	
	public T max(){
		return heap[1];
	}
	public boolean isEmpty(){
		return size==0;
	}
    /**
     * Inserts an object in this heap.
     * throws IllegalStateException if heap capacity is exceeded.
     * param object The object to insert.
     */
    public void insert(T object) {
        // Ensure object is not null
        if (object == null) throw new IllegalArgumentException();
		resize();
        // Check available space
     
        // Place object at the next available position
        heap[++size] = object;
		
		if(object instanceof Song){
			Song a=(Song)object;
			Songposition[a.getId()]=size;
		}
        // Let the newly added object swim
        swim(size);
    }
    /**
     * Removes the object at the root of this heap.
     * throws IllegalStateException if heap is empty.
     * return The object removed.
     */
    public T getMax() {
        // Ensure not empty
        if (size == 0) throw new IllegalStateException();
        // Keep a reference to the root object
        T object = heap[1];
        // Replace root object with the one at rightmost leaf
        if (size > 1) heap[1] = heap[size];
        // Dispose the rightmost leaf
        heap[size--] = null;
		if(object instanceof Song){
			Song a=(Song)object;
			Songposition[a.getId()]=0;
		}
        // Sink the new root element
        sink(1);
        // Return the object removed
        return object;
    }
    /**
     * Shift up.
     */
	 public T remove(int id){
		 T object=null;
		 int removeposition=Songposition[id];
		 if(removeposition==0){
			 System.out.println("there is no song with this id in the priority queue");
			 throw new NullPointerException();
		 }else{
		 
		
		
			 
		 if(removeposition==size){
			 heap[size--]=null;
		 }else{
		 
		 object=heap[removeposition];
		 heap[removeposition]=heap[size];
		 heap[size--]=null;
		 
		 swim(removeposition);
		 sink(removeposition);
		 }
		 }
		 
		
		 
		 
		 return object;
		 
		 
		 
	 }
	private void resize(){
		 if(size==3*heap.length/4){
			 
			 
			 T [] newArray=(T []) new Object[2*heap.length];
			 for(int i=0;i<=size;i++){
				 newArray[i]=heap[i];
			 }
			 heap=newArray;
		 }
	 }
	 public T printArr(int id){
		 return heap[id];
	 }
	
		 
		 
		 
	 
    private void swim(int i) {
        while (i > 1) {  //if i root (i==1) return
            int p = i/2;  //find parent
			//System.out.println("I made it here");
			//System.out.println(i+" "+p);
			//System.out.println(size+" "+heap.length);
            int result = cmp.compare(heap[i],heap[p]);  //compare parent with child
			
            if (result <= 0) return;  
			
            swap(i, p);
			
			
			if(heap[i] instanceof Song){
				songSwap(p,i);
			}
			
			
            i = p;
        }
    }
    /**
     * Shift down.
     */
    private void sink(int i){
        int left = 2*i, right = left+1, max = left;
        // If 2*i >= size, node i is a leaf
        while (left <= size) {
            // Determine the largest children of node i
            if (right <= size) {
                max = cmp.compare(heap[left], heap[right]) < 0 ? right : left;
            }
            // If the heap condition holds, stop. Else swap and go on.
            if (cmp.compare(heap[i], heap[max]) >= 0) return;
            swap(i, max);
			if(heap[i] instanceof Song){
			     songSwap(max,i);
				 
		}
            i = max; left = 2*i; right = left+1; max = left;
        }
    }
    
    /**
     * Interchanges two array elements.
     */
    private void swap(int i, int j) {
        T tmp = heap[i];
        heap[i] = heap[j];
        heap[j] = tmp;
		
		
    }
	void songSwap(int i,int j){
		Song a=(Song)heap[i];
		Song b=(Song)heap[j];
		int tmp=Songposition[a.getId()];
		Songposition[a.getId()]=Songposition[b.getId()];
		Songposition[b.getId()]=tmp;
	}
		
		
    public void print() {
        for (int i=1; i<=size; i++){
            System.out.print(heap[i]+ " ");
        }
        System.out.println();
    }
    boolean empty(){
        return size == 0;
    }
    
    
}
